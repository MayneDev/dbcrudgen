...
import com.marvik.apis.dbcrudgen.parser.android.httpresponse.AndroidTableHttpResponseTemplateParser;
...
public class AndroidCRUDCreator extends CrudCreator {
    ...
    public void createProject(Database database) {
        ...
        createDirectory(projectFilesDefaultStorageDirectory + NativeUtils.getFileSeparator() + contentProvidersPath);
        ...
        String sqliteOpenHelperPath = NativeUtils.transformPath(NativeTemplateTags.DOT, NativeUtils.getFileSeparator(), sqliteOpenHelperSubclassPackage);
        createDirectory(
                projectFilesDefaultStorageDirectory + NativeUtils.getFileSeparator() + sqliteOpenHelperPath);

        // Database table package

        String tableSchemasPackage = getAndroidDatabaseConfiguration().getTablesSchemasPackage();
        String tablesSchemasStorageLocation = NativeUtils.transformPath(NativeTemplateTags.DOT, NativeUtils.getFileSeparator(), tableSchemasPackage);
        createDirectory(
        ...
        tablesCRUDStorageLocation = NativeUtils.transformPath(NativeTemplateTags.DOT, NativeUtils.getFileSeparator(), tablesCRUDStorageLocation);
        createDirectory(
        ...
        createContentProviderSourceFile(projectFilesDefaultStorageDirectory, contentProvidersPath, database);

        // create SQLiteOpenHelper Subclass
        createSQLiteOpenHelperSourceFile(projectFilesDefaultStorageDirectory, sqliteOpenHelperPath);
        ...
        // create tables http responses model info classes
        String tablesHttpResponsesPackage = getAndroidDatabaseConfiguration().getTablesHttpResponsesPackage();
        String tablesHttpResponsesPath = NativeUtils.transformPath(NativeTemplateTags.DOT, NativeUtils.getFileSeparator(), tablesHttpResponsesPackage);
    ...
    private void createTableModelInfoClassesSourceFiles(Database database, String projectFilesDefaultStorageDirectory,
        ...
        for (Table table : database.getTables()) {
            ...
            String tableModelSourceFilePackageFilePath = tablesModelInfoPackage;
    ...
    private void createHttpResponsesClassesSourceFiles(Database database, String filesStorageDir, String packageName, String tablesHttpResponsesPackage) {

        AndroidTableHttpResponseTemplateParser androidHttpResponseTemplateParser = new AndroidTableHttpResponseTemplateParser();
        ...
        for (Table table : database.getTables()) {
            ...
            // package where this table model info class will be saved
            String httpResponseSourceFilePath = tablesHttpResponsesPackage;
            httpResponseSourceFilePath = NativeUtils.transformPath(NativeTemplateTags.DOT, NativeUtils.getFileSeparator(),
                    httpResponseSourceFilePath);

            // the table model source file
            String tableModelSourceFile = filesStorageDir + NativeUtils.getFileSeparator()
